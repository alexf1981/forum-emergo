...
(function (global) {
    ...
    /**
     * Calculates the result of a battle.
     * @param {Object} hero - The hero object.
     * @param {Object} quest - The quest object.
     * @returns {Object} result - { success: boolean, earnedXp, earnedGold, dmgTaken, lootMsg, newItems, newLvl, newStr, newXp, newHp }
     */
    function calculateBattleResult(hero, quest) {
        // Calculate Powers
    ...
    function createHabit(habits, text, type, isOneTime) {
    ...
    function updateHabit(habits, id, updates) {
        return habits.map(h => h.id === id ? { ...h, ...updates } : h);
    }

    /**
     * Toggles a habit's completion status for a given date.
     * calculates rewards/penalties based on habit type.
     * @returns {Object} { newHabits, newStats, notifications }
     */
    function processHabitToggle(habits, stats, id, dateString) {
        let notifications = [];
        let newStats = { ...stats };

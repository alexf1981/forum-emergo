        ...
        <div>
            <div className="tavern-bg">
                ...
                <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                    <button className="btn" onClick={onRecruit}>Nieuwe Held Rekruteren ({formatNumber(100)}g)</button>
                </div>
                <div className="heroes-grid" style={{ marginTop: '20px' }}>
                    {heroes.map(h => (
                        <div key={h.id} className="card hero-card">
                            <div className="card-title">{h.name} <span style={{ fontSize: '0.8em', color: '#aaa' }}>Lvl {formatNumber(h.lvl)}</span></div>
                            <div>XP: {formatNumber(h.xp)} / {formatNumber(h.lvl * 100)}</div>
